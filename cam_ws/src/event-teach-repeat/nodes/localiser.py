...
#         self.frame_times = deque(maxlen=window_size)  # stores timestamps
#         self.frames_count = 0

#     def update(self):
#         """Call this every frame to record its timestamp."""
#         now = time.perf_counter()  # high-resolution timestamp in seconds
#         self.frame_times.append(now)
#         self.frames_count += 1

#     def get_fps(self):
#         """Compute FPS over the window. Returns 0 if not enough frames."""
#         if self.frames_count < self.window_size:
#             return 0.0
#         oldest = self.frame_times[0]
#         newest = self.frame_times[-1]
#         duration = newest - oldest  # seconds
#         if duration == 0:
#             return 0.0
#         return (self.window_size - 1) / duration
...
class teach_repeat_localiser:
	def __init__(self):
		...
		# self.fps_estimator = FramerateEstimator(window_size=50)
		# self.fps = []
	...
	def save_data_at_goal(self, pose, goal_odom, goal_world, theta_offset, path_offset):
		...
		with open(self.save_dir+('time_profile.csv'), 'a') as time_file:
			message = f"{self.process_time},{self.offset_time} \n"
	...
	def process_image_data(self, msg):
		if self.running:
			...
			self.process_time = time_after-time_before
	...
	def make_new_goal(self, rotation_correction=0.0, path_correction=1.0):
		...
		if state == GOAL_STATE.finished:
			...
			with open("/home/qcr/gokul/dom_op.txt", "w") as f:
	...
	def do_continuous_correction(self):
		if self.last_odom_pose is not None and self.goal_index > 0:
			...
			time_after = time.perf_counter()
			self.offset_time = time_after-time_before
			...
			# self.now = time.perf_counter()
			# if self.now - self.last_print >= 1.0:  # print every 1 second
			# 	fps_val = self.fps_estimator.update()
			# 	fps_val = self.fps_estimator.get_fps()
