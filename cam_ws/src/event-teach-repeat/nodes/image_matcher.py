...
class image_matcher:
		...
		if self.use_old_dataset_format:
			...
			if os.path.exists(self.load_dir+'params.txt'):
				...
				if self.resize == tuple(params['resize']) and self.patch_size == tuple(params['patch_size']):
					...
					# self.images = self.load_events_raw(image_files)
					print("Loaded events!!!")
				else:
	...
	def load_images_resize_norm(self, image_files):
		...
		imgs =  [image_processing.patch_normalise_pad(cv2.resize(image,  self.resize[::-1], interpolation=cv2.INTER_AREA), self.patch_size) for image in images]
		return [np.pad(image, ((0,),(int(image.shape[1]/2),)), mode='constant', constant_values=0) for image in imgs]
	...
	def load_events_raw(self, image_files):
		...
		# resized_images = [cv2.resize(cd_frame, None, fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA) for cd_frame in raw_images]
	...
	def match_image(self, request):
		image = ros_numpy.numpify(request.normalisedImage) # image_processing.msg_to_image(request.normalisedImage)
		...
		if debug_image is not None:
			...
			self.pub_image_match_debug.publish(ros_numpy.msgify(Image, debug_image,'bgr8'))
	...
	def match_image_old(self, request):
		image = ros_numpy.numpify(request.normalisedImage) # image = image_processing.msg_to_image(request.normalisedImage)
		image_index = request.imageIndex.data
		start_range, end_range = self.clamp_search_range_to_bounds(image_index, request.searchRange.data)
		centre_image_index = image_index - start_range
		match_data = [[] for i in range(end_range - start_range)]
		...
		ref_image = np.concatenate(self.images[start_range:end_range], axis=1)
		offsets_data, correlations_data, debug_image = image_processing.concat_fft_match_images_debug(ref_image, image, len(self.images[start_range:end_range]), centre_image_index, self.subsampling)
		...
		# 	img_index = start_range + i
		# 	if i == centre_image_index:
		# 		offset, corr, debug_image = image_processing.fft_match_images_debug(self.images[img_index], image, self.subsampling)
		# 		match_data[i] = (offset, corr)
		# 	else:
		# 		match_data[i] = image_processing.fft_match_images_debug(self.images[img_index], image, self.subsampling)
