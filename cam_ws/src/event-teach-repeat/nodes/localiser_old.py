...
# D415 # EVK4 # Miro (Stereo-stitched?)
FIELD_OF_VIEW_DEG =  35.943 # 69.636 #  2*60.6 + 2*27.0
IMAGE_WIDTH =  80 # 115 # px
CORR_SUB_SAMPLING = 1
...
class teach_repeat_localiser:
	...
	def setup_parameters(self):
		# Teach Repeat Params
		self.rotation_correction_gain = rospy.get_param('~rotation_correction_gain', 0.003)
		self.path_correction_gain = rospy.get_param('~path_correction_gain', 0.0001)
		...
		perturbation_dg = 5
		perturbation_rd = math.radians(perturbation_dg)
		perturbation_qn = quaternion_from_euler(0, 0, perturbation_rd)
		rotation_perturbation = PyKDL.Rotation.Quaternion(0, 0, perturbation_qn[2], perturbation_qn[3])
		self.zero_goal = tf_conversions.toMsg(PyKDL.Frame(R=rotation_perturbation, V=PyKDL.Vector(0, 0, 0)))
		self.previous_correction = PyKDL.Frame(R=PyKDL.Rotation.Quaternion(0,0,0,1), V=PyKDL.Vector(0, 0, 0))
		self.perturb_goals()
		...
		if not os.path.isdir(self.save_dir+'offset_correction/'):
			os.makedirs(self.save_dir+'offset_correction/')
	...
	def perturb_goals(self):
		zero_position = tf_conversions.fromMsg(self.zero_goal)
		for i, pose in enumerate(self.poses):
			goal = tf_conversions.fromMsg(pose)
			perturbed_goal = zero_position * goal
			self.poses[i] = tf_conversions.toMsg(perturbed_goal)
	...
	def start(self):
		...
		if self.zeroth_goal:
			print('Publishing zeroth goal\n')
			
			goal = Goal()
			goal.pose.pose = self.zero_goal
			goal.stop_at_goal.data = False
			...
			zero_pose_perturbed = tf_conversions.fromMsg(self.zero_goal)
			delta_frame_zero = current_pose_odom.Inverse() * zero_pose_perturbed
			
			print('Waiting for zeroth goal to be executed\n')
			while delta_frame_zero.p.Norm() > 0.17:
				self.mutex.acquire()
				current_pose_odom = tf_conversions.fromMsg(self.last_odom_pose)
				self.mutex.release()
			...
			print('Done executing zeroth goal\n')				
			self.zeroth_goal = False
	...
	def save_data_at_goal(self, pose, goal_odom, goal_world, theta_offset, path_offset):
		...
		if self.last_full_res_image is not None:
	...
	def save_corrections(self, theta_offset, path_offset, img_seq, img_recv_time, vision_time, total_time, rotation_corr, path_corr, time_at_recv, offset):
		# publish offset to tf
		# self.tf_pub.sendTransform((offset.position.x, offset.position.y, offset.position.z), (offset.orientation.x, offset.orientation.y, offset.orientation.z, offset.orientation.w), rospy.Time.now(), 'map', 'odom')
		# publish current corrections
		time_stamp = rospy.Time.now().to_sec()
		message_as_text = json.dumps({'time_stamp': time_stamp, 'theta_offset': theta_offset, 'path_offset': path_offset, 'rotation_corr': rotation_corr, 'path_corr': path_corr, 'img_recv_time': img_recv_time, 'vision_time': vision_time, 'total_time': total_time, 'time_at_recv': time_at_recv, 'offset': offset})
		with open(self.save_dir+('offset_correction/%06d_correction.txt' % img_seq), 'w') as correction_file:
			correction_file.write(message_as_text)
	...
	def process_image_data(self, msg):
		if self.running:
			...
			full_image = ros_numpy.numpify(msg) # image_processing.msg_to_image(msg)
			...
			if not self.discrete_correction:
				self.do_continuous_correction(msg.header.stamp.to_sec(), n, time_at_recv)
			self.mutex.release()
		else:
			self.mutex.acquire()
			# self.last_image = image_processing.patch_normalise_image(image_processing.msg_to_image(msg), (9,9), resize=self.resize)
			self.last_image = image_processing.patch_normalise_image(ros_numpy.numpify(msg), (9,9), resize=self.resize)
			self.mutex.release()

	def process_image_data_old(self, msg):
		if self.running:
			n = msg.header.seq
			time_at_recv = rospy.Time.now().to_sec()
			...
			if self.save_full_res_images:
				cv2.imwrite(self.save_dir+('full/%06d.png' % n), full_image)
			cv2.imwrite(self.save_dir+('norm/%06d.png' % n), np.uint8((64 + normalised_image)))
	...
	def do_continuous_correction(self, img_recv_time, img_seq, time_at_recv):
		if self.last_odom_pose is not None and self.goal_index > 0:
			...
			vision_time = rospy.Time.now().to_sec()
			...
			print(offsets)
			degrees = [math.degrees(rad) for rad in rotation_offsets]
			print(rotation_offsets, degrees, '\n')
			...
			total_time = rospy.Time.now().to_sec()
			self.save_corrections(self.sum_theta_correction, self.sum_path_correction, img_seq, img_recv_time, vision_time, total_time, rotation_correction, path_correction, time_at_recv, math.degrees(rotation_offsets[1]))
	...
	def calculate_image_pose_offset(self, image_to_search_index, half_search_range=0):
		if self.last_image is not None:
			match_request = ImageMatchRequest(image_processing.numpy_to_image_msg(self.last_image), UInt32(image_to_search_index), UInt32(half_search_range))
			...
			print(FIELD_OF_VIEW_DEG, IMAGE_WIDTH, CORR_SUB_SAMPLING)
			print(match_response.offsets.data)
