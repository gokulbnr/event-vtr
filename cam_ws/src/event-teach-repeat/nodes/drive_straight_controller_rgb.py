...
class drive_straight_controller():
	...
	def __init__(self):
		# super().__init__("")
		# self.event_frame_gen.set_fps(150)
	...
	def setup_parameters(self):
		...
		if self.teach_mode is False:
			load_dir = rospy.get_param('/drive_straight_controller/load_dir','/home/qcr/gokul/tnr-toy-data/2025-02-27_10:56:31/')
	...
	def setup_subscribers(self):
		...
		# self.sub_events = rospy.Subscriber("image_publisher_1740993327043095461/image_raw", Image, self.process_event_frames, queue_size=1, buff_size=2**22)
		...
		if self.odom_ready is True:
			...
			if self.teach_mode is True:
				...
				with open(self.rgb_pose_save_dir+id+'_pose.txt', 'w') as pose_file:
					pose_file.write(message_as_text)
				message_as_text = json.dumps({'time_stamp': time_stamp, 'theta_offset': offset_deg, 'time_at_recv': time_at_recv})
				with open(self.rgb_offset_correction+'%06d_correction.txt' % self.rgb_count, 'w') as correction_file:
					correction_file.write(message_as_text)
...
if __name__ == "__main__":
	rospy.init_node("drive_straight_controller_rgb")
