...
def numpy_to_image_msg(np_image, frame_id="camera"):
    """
    Convert a single-channel float64 numpy array to ROS Image message.
    """
    if np_image.dtype != np.float64:
    ...
    msg.height = np_image.shape[0]
    msg.width = np_image.shape[1]
    msg.encoding = '64FC1'       # single-channel float64
    msg.is_bigendian = 0
    msg.step = np_image.shape[1] * 8  # bytes per row: 8 bytes for float64
...
# def fft_match_images_debug(image, template_image, subsampling=1):
# 	# padded_image = np.pad(image, ((0,0), (int(image.shape[1]/2), int(image.shape[1]/2))), mode='constant')
# 	padded_width = image.shape[1]
# 	qry_img = np.flipud(np.fliplr(template_image))
# 	corr= fftconvolve_torch(torch.tensor(image), torch.tensor(qry_img.copy()), mode='valid')
# 	corr = corr.numpy()
# 	corr_normalized = (corr - np.min(corr)) / (np.max(corr) - np.min(corr))
# 	corr_slice = corr_normalized[:,80:240]
# 	max_index_flat = np.argmax(corr_slice)
# 	max_x, max_y = np.unravel_index(max_index_flat, corr_slice.shape)
# 	raw_image = image[:,padded_width//4:3*padded_width//4]
# 	debug_image = visualize((template_image+64).astype(np.uint8), (raw_image+64).astype(np.uint8), max_y+80)
# 	return (max_y+80)-160, corr_normalized[max_x, max_y+80], debug_image

def concat_fft_match_images_debug(image, template_image, num_imgs, centre_image_index, subsampling=1):
	img_width = (image.shape[1] // num_imgs)
	qry_img = np.flipud(np.fliplr(template_image))
	corr= fftconvolve_torch(torch.tensor(image), torch.tensor(qry_img.copy()), mode='valid')	
	corr = corr.numpy()[0,:-1]
	cor_arr = []
	...
	for i in range(num_imgs):
		cor = corr[i*img_width + 0 : i*img_width + img_width//2]
		max_index_flat = np.argmax(cor)
		max_y = np.unravel_index(max_index_flat, cor.shape)[0]
		cor_arr.append(cor[max_y])
		offset_arr.append(max_y-img_width//4)

		if max_y - img_width//4 < -40 or max_y - img_width//4 > 40:
			print("corr shape: ", cor.shape, " offset: ", max_y - img_width//4)

		if i == centre_image_index:
			raw_image = image[:,i*img_width + img_width//4 : i*img_width + 3*img_width//4]
			debug_image = visualize((template_image+64).astype(np.uint8), (raw_image+64).astype(np.uint8), max_y)
	# print(offset_arr)
	return offset_arr, cor_arr, debug_image

def fftconvolve_torch(a, b, mode='same'):
	# Determine the size of the output based on the mode
	if mode == 'full':
		out_size = [s1 + s2 - 1 for s1, s2 in zip(a.shape, b.shape)]
	elif mode == 'same':
		out_size = a.shape
	elif mode == 'valid':
		out_size = [s1 - s2 + 1 for s1, s2 in zip(a.shape, b.shape)]
	...
	# Perform FFT of both inputs, with appropriate zero-padding
	a_fft = torch.fft.rfft2(a, fft_size)
	b_fft = torch.fft.rfft2(b, fft_size)

	# Multiply in the frequency domain
	result_fft = a_fft * b_fft

	# Perform inverse FFT to get the convolution result
	result = torch.fft.irfftn(result_fft, fft_size)

	# Slice the result to the desired output size
	if mode == 'full':
		return result
	elif mode == 'same':
		start = [(fs - os) // 2 for fs, os in zip(result.shape, out_size)]
		end = [start[i] + out_size[i] for i in range(len(out_size))]
		slices = tuple(slice(start[i], end[i]) for i in range(len(out_size)))
		return result[slices]
	elif mode == 'valid':
		start = [(fs - os) for fs, os in zip(result.shape, out_size)]
		slices = tuple(slice(start[i], start[i] + out_size[i]) for i in range(len(out_size)))
		return result[slices]
...
def visualize(qry, match, match_y):

	if len(qry.shape) <= 2:
		match_color = cv2.cvtColor(match, cv2.COLOR_GRAY2BGR)
		qry_color = cv2.cvtColor(qry, cv2.COLOR_GRAY2BGR)
	else: 
		match_color = match
		qry_color = qry
		
	cv2.line(qry_color, (int(qry.shape[1] / 2), 0),
				(int(qry.shape[1] / 2), qry.shape[0]), (0, 255, 0), 2)
	cv2.line(match_color, (match_y, 0), (match_y, match.shape[0]), (0, 255, 0), 2)

	combined = cv2.vconcat([qry_color, match_color])
